...
def convert_coords(coords: list):
    ...
    # return [int(x1), int(y2), int(x2), int(y1)]
...
def resolve_lost_tracks(lost_tracks_prev: dict, lost_tracks_next: dict):
    ...
    while not np.all(np.isinf(similarity_matrix)):
        ...
        if np.argmax(similarity_matrix)>=0.9999:
    ...
    for frame_number in range(1, total_frames):
        ...
        for id in list(
            ...
            if id not in distances:
                ...
                if distances[id]>5:
                    print(prev_id_to_next_id)
                    print("id",id)
                    print(prev_frame_predictions[id])
                    print(next_frame_predictions[id],"\n")
                    matrix = cosine_similarity(list(lost_tracks_prev.values()), list(lost_tracks_next.values()))
                    print(matrix)
                    print(distances)
                    print(matrix.shape)
                    raise ValueError
...
if __name__ == "__main__":
    adam_tracking_v2(
        model_path="MAR8\\weights\\best.pt",
        input_video_path="sheeps_30sec.mp4",
        # input_video_path="output_video4.avi",
        output_video_path="result\\adam_tracking_v2/evaluation_tracked3.mp4",
        augment=True,
